#!/usr/bin/env bash
# Rachit's launcher (2026-05-09).
#
# RL on OLMo2-4B-stage1-50B pretrain checkpoints — 72h, 4 H100 each.
#   Inner script:  examples/grpo_trainer/run_olmo2-4b_openmath.sh
#                  (auto-derives OLMO_CHECKPOINT from STEP_NUM:
#                   /n/netscratch/dam_lab/Everyone/rl_pretrain/
#                     OLMo2-4B-stage1-50B/step${STEP_NUM}-hf)
#   SLURM wrapper: examples/grpo_trainer/run_olmo2-4b_openmath_slurm.sh
#                  (forwards STEP_NUM as $1 → exported into the inner script)
#   Pretrain steps: 2k, 5k, 10k, 14k
#   save_freq=50, test_freq=25 (configured in inner script)
#   Exp name: olmo2_4b_step{N}_omi_n32
#   Job names: rl-4b-2k, rl-4b-5k, rl-4b-10k, rl-4b-14k
#
# Usage: bash examples/grpo_trainer/launch_rachit_05_09.sh
#        (runnable from anywhere — the script cd's into VERL_DIR itself)

set -eo pipefail

# ============================================================================
# USER CONFIG — change these for your setup
# ============================================================================
SLURM_ACCOUNT="kempner_barak_lab"
VERL_DIR="/n/home05/sqin/rl_pretrain/verl"
CONDA_ENV="/n/holylabs/dam_lab/Lab/brachit/envs/bin/activate"
MAIL_USER="tqin@g.harvard.edu"
# ============================================================================

cd "${VERL_DIR}"
SLURM_WRAPPER="examples/grpo_trainer/run_olmo2-4b_openmath_slurm.sh"

step_to_suffix() {
    local step=$1
    if (( step >= 1000 )); then
        echo "$((step / 1000))k"
    else
        echo "${step}"
    fi
}

mkdir -p logs
COUNT=0

PRETRAIN_STEPS=(2000 5000 10000 14000)

for STEP in "${PRETRAIN_STEPS[@]}"; do
    SUFFIX=$(step_to_suffix $STEP)

    echo "Submitting rl-4b-${SUFFIX} (RL on OLMo2-4B-50B step ${STEP}) — 72h, 4 H100"
    sbatch --account=${SLURM_ACCOUNT} --partition=kempner_h100 \
        --mail-type=FAIL --mail-user=${MAIL_USER} \
        --job-name="rl-4b-${SUFFIX}" --time=72:00:00 \
        --gpus-per-node=4 --cpus-per-task=96 \
        --export=ALL,VERL_DIR=${VERL_DIR},CONDA_ENV=${CONDA_ENV},STEP_NUM=${STEP} \
        ${SLURM_WRAPPER} ${STEP}
    COUNT=$((COUNT + 1))
    sleep 1
done

echo ""
echo "Submitted ${COUNT} jobs total."
