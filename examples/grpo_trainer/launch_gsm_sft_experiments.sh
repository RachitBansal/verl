...
sbatch --account=${SLURM_ACCOUNT} --mail-type=FAIL --mail-user=${MAIL_USER} --job-name="comb-${SUFFIX}" --time=48:00:00 \
...
sbatch --account=${SLURM_ACCOUNT} --mail-type=FAIL --mail-user=${MAIL_USER} --job-name="rgsm-il-${SUFFIX}" --time=36:00:00 \
...
# ============================================================================
# 3. Resume all RMATH (1B-50B) — 24 hours
# ============================================================================
...
for STEP in "${STEPS_RMATH_50B[@]}"; do
    ...
    echo "Submitting rmath-${SUFFIX} (MATH duplicated, 50B step ${STEP}) — resume, 24h"
    sbatch --account=${SLURM_ACCOUNT} --mail-type=FAIL --mail-user=${MAIL_USER} --job-name="rmath-${SUFFIX}" --time=24:00:00 \
...
# ============================================================================
# 4. Resume 60B MATH (de-duplicated) — all except 17k — 8 hours
# ============================================================================
...
for STEP in "${STEPS_60B_MATH[@]}"; do
    ...
    echo "Submitting math60-${SUFFIX} (MATH de-duplicated, 60B step ${STEP}) — resume, 8h"
    sbatch --account=${SLURM_ACCOUNT} --mail-type=FAIL --mail-user=${MAIL_USER} --job-name="math60-${SUFFIX}" --time=08:00:00 \
...
# ============================================================================
# 5. Resume 60B RMATH (duplicated) — 5k, 10k, 15k — 48 hours
# ============================================================================
