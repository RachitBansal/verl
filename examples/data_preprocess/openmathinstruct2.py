...
1. train_subset.parquet: Random 5k samples from entire OpenMathInstruct-2
2. train_gsm8k.parquet: GSM8K questions for training (all except validation)
3. val_gsm8k.parquet: 1k GSM8K questions for validation
...
if __name__ == "__main__":
    ...
    print(f"train_subset size: {len(train_subset)}", flush=True)
    ...
    # Save train_subset.parquet
    output_file = os.path.join(local_dir, "train_subset.parquet")
    train_subset.to_parquet(output_file)
    print(f"✓ Saved train_subset.parquet ({len(train_subset)} examples)", flush=True)
    ...
    # Save one example from each dataset as JSON for reference
    example = train_subset[0]
    example_file = os.path.join(local_dir, "train_subset_example.json")
    with open(example_file, "w") as f:
        json.dump(example, f, indent=2)
    print(f"✓ Saved example to train_subset_example.json", flush=True)
    ...
    print(f"  - train_subset.parquet: {len(train_subset)} examples (random subset from all)", flush=True)
