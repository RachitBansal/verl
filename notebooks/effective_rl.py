...
def plot_first_row(df: pd.DataFrame, output_path: Path, relative: bool = False):
    ...
    PANEL_KS = [1, 32]
    panel_specs = [("gsm", "rl", k) for k in PANEL_KS] + [("math", "rl", k) for k in PANEL_KS]

    fig, axes = plt.subplots(1, len(panel_specs), figsize=(3.7 * len(panel_specs), 3.6))
    axes = axes.ravel()

    for col_idx, (ds_key, tname, k) in enumerate(panel_specs):
        ax = axes[col_idx]
        cfg = DATASETS[ds_key]
        sub = df[df["dataset"] == ds_key].dropna(subset=[
            f"omi_pass_{k}", "base_test_pass_1", "base_test_pass_32",
        ...
        if not sub.empty:
            ...
            for series_k in Y_DELTA_KS:
                ...
                delta_abs = (sub_d[f"{tname}_test_pass_{series_k}"] - base_vals) * 100
                ...
                if series_k == 32:
                    ...
                    ax.scatter(sub_d["_x"], sub_d["_y"], facecolors="none",
                               ...
                               linewidths=1.6, zorder=10)
                if len(sub_d) >= 2:
                    m, b = np.polyfit(sub_d["_x"], sub_d["_y"], 1)
                    x_line = np.linspace(sub_d["_x"].min(), sub_d["_x"].max(), 100)
                    ls = "-" if series_k == 32 else "--"
                    ax.plot(x_line, m * x_line + b, color=cfg["color"],
                            linewidth=1.5, linestyle=ls, alpha=0.7, zorder=5)
