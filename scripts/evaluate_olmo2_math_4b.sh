#!/bin/bash
#
# Evaluation script for OLMo-2 4B base model on GSM-8K and MATH datasets
# Uses 8-shot prompting (base models need few-shot examples).
#
# Usage:
#   bash scripts/evaluate_olmo2_math_4b.sh
#
# Author: Generated for OLMo-2 4B base evaluation
# Date: $(date +%Y-%m-%d)
...
MODEL_PATH_DEFAULT=/n/netscratch/dam_lab/Everyone/rl_pretrain/OLMo2-4B-stage1-50B/step5000-hf  # arg1
MODEL_NAME_DEFAULT="4B-stage1-50B-step5000"  # arg2
...
NNODES=1
N_GPUS_PER_NODE=2
TENSOR_PARALLEL_SIZE=2  # 4B model: split across 2 GPUs
...
N_SHOT=8  # Number of few-shot examples (base model — needs prompting)
...
echo "================================================"
echo "OLMo-2 4B Base Math Evaluation Script"
echo "================================================"
...
if [ "${EVAL_GSM8K}" = true ]; then
    ...
    if [ ! -f "${GSM8K_OUTPUT}" ]; then
        python3 -m verl.trainer.main_generation \
            ...
            ray_kwargs.ray_init.num_cpus=20
...
if [ "${EVAL_GSM8K}" = true ]; then
    ...
    python3 -m verl.trainer.main_eval \
        ...
        ray_kwargs.ray_init.num_cpus=20 | tee "${OUTPUT_DIR}/gsm8k_results.txt"
fi
...
if [ "${EVAL_MATH}" = true ]; then
    ...
    python3 -m verl.trainer.main_eval \
        ...
        ray_kwargs.ray_init.num_cpus=20 | tee "${OUTPUT_DIR}/math_results.txt"
fi
