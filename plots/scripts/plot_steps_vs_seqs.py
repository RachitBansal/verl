...
y: first validation step with AIME 1983-2024 mean@1 >= 50%, log.
...
Usage: python plot_steps_vs_seqs.py steps_to_50_seqs.csv out.png [--prefix]
"""
import sys, os
import numpy as np
import pandas as pd
...
long_enough = (df["last_val_step"].fillna(0) >= SHORT_STEPS) | df["steps_to_50"].notna()   # short runs count once they reach 50%


def pick_fastest(s):
    return s.sort_values(["steps_to_50", "last_val_step"], na_position="last",
...
def collapse(src, keys, series, k_from=None):
    ...
    for key, s in src.groupby(keys):
        ...
        rows.append(dict(series=series, seqs=int(p["seqs"]), lr=p["lr"],
                         ...
                         steps=p["steps_to_50"], last=p["last_val_step"], max_val=p["max_val"], run=p["name"]))
...
pd.concat([best.assign(role="plain_fastest_lr"), matched.assign(role="plain_lr1e-5"),
           ...
           pre.assign(role="downsample_prefix")]).to_csv(os.path.join(os.path.dirname(os.path.abspath(csv_in)), os.path.basename(png_out).replace(".png", "_table.csv")), index=False)
...
allx = pd.concat([plain["seqs"], ds["seqs"], pfix["seqs"], pre["seqs"]])
xs = np.array([allx.min() / 1.4, allx.max() * 1.4])
ax.plot(xs, s0 * b0 / xs, ls=":", lw=1.2, color=MUTED, zorder=1)
ax.annotate("perfect scaling (steps ∝ 1/sequences)", (xs[1], s0 * b0 / xs[1]), xytext=(-4, 6),
...
for _, r in best.iterrows():
    ax.annotate(f"lr {r['lr']:g}", (r["seqs"], r["steps"]), xytext=(0, -15), textcoords="offset points",
...
for _, r in pf_hit.iterrows():
    ax.annotate(f"plain, fixed scaling, lr {r['lr']:g}", (r["seqs"], r["steps"]), xytext=(12, -4),
...
HIT_POS = {4: dict(xytext=(0, 9), ha="center"), 16: dict(xytext=(-10, -4), ha="right")}
...
CENS_POS = {2: dict(xytext=(0, -13), ha="center") if SHOW_PREFIX else dict(xytext=(-10, -3), ha="right"),
...
ticks = sorted(set(pd.concat([plain["seqs"], ds["seqs"], pfix["seqs"]]).unique()))
